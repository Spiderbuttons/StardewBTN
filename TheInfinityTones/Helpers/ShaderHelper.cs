...
public static class ShaderHelper
{
    ...
    private static Dictionary<string, FileSystemWatcher> ShaderWatchers = new();
    private static Dictionary<string, FileSystemWatcher> SourceWatchers = new();

    public static void BeginDefault(this SpriteBatch b)
    {
        b.Begin(
    ...
    public static void BeginWithShader(this SpriteBatch b, Effect shader)
    {
        b.Begin(
            sortMode: SpriteSortMode.Deferred,
            blendState: BlendState.AlphaBlend,
    ...
    public static void WatchShader(string shaderName, Action<Effect> onChange, [CallerFilePath] string? callerFilePath = null)
    {
        ...
        watcher.Changed += (sender, e) =>
        {
            try
            {
                ...
                onChange(newEffect);
                ModEntry.ModMonitor.Log($"Shader {shaderName} reloaded successfully.", level: (LogLevel)7);
            }
            catch (Exception ex)
            {
